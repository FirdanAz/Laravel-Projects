...
<body>
    <div class="container mt-5">
        <div class="row d-flex justify-content-center align-items-center">
            <div class="col-md-8">
                <form method="post" action="/wisata/update/{{ $wisata->id }}">
                    @csrf
                    ...
                    <div class="tab">
                        <select class="form-select" name="location_id" id="">
                            @foreach ($location as $location)
                                @if (old('location_id', $wisata->location_id == $location->id))
                                    <option name="location_id" value="{{ $location->id}}" selected>{{ $location->city }}</option>        
                                @else
                    ...
                    <div class="tab">
                        <h6>Jam Buka</h6>
                          <p>
                            <input type="time" placeholder="Masukan Jam Buka..." oninput="this.className = ''" name="open_time" class="test" required id="open_time" value="{{old('open_time', $wisata->open_time)}}"></p>
                          
                    </div>
                    <div class="tab">
                        <h6>Jam Tutup</h6>
                          <p>
                            <input type="time" placeholder="Masukan Jam Tutup..." oninput="this.className = ''" name="close_time" class="test" required id="close_time" value="{{old('close_time', $wisata->close_time)}}"></p>
